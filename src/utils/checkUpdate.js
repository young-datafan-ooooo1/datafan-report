      ...
      packageJson(name, {
        ...
        .then(res => {
          if (semver.lt(require(`${name}/package.json`).version, res.version)) {
            ...
            str.includes(name.replace('@sense70/', '')) ? NEED_PULL.push(name) : NEED_UPDATE.push(name)
          }
          resolve()
        })
        .catch(e => reject(e))
...
Promise.all(PROMISE_SEQUENCE).then(() => {
  if (NEED_PULL.length > 0) {
    console.log(chalk.bold.green('🦊 以下包需要更新，不做任何处理。'))
  ...
  if (NEED_UPDATE.length > 0) {
    console.log(chalk.bold.green('🚀 以下包需要更新，将自动执行yarn upgrade命令'))
